...
class Simulator(object):
    ...
    def __init__(self, env, size=None, update_delay=2.0, display=True, log_metrics=False, optimized=False, label_epsilon = None, label_alpha = None):
        self.env = env
        ...
        if self.log_metrics:
            ...
            if a.learning:
                if self.optimized: # Whether the user is optimizing the parameters and decay functions
                    self.log_filename = os.path.join("logs", "sim_improved-learning-epsilon-{}-alpha-{}.csv".format(label_epsilon, label_alpha))
                    self.table_filename = os.path.join("logs","sim_improved-learning-epsilon-{}-alpha-{}.txt".format(label_epsilon, label_alpha))                            
                else: 
